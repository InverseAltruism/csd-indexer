...
for (const r of await store().all("SELECT domain, COUNT(*) n FROM proposals GROUP BY domain"))
  myProp.set(r.domain, Number(r.n));
...
const totProp = Number((await store().get("SELECT COUNT(*) n FROM proposals")).n);
const totAtt = Number((await store().get("SELECT COUNT(*) n FROM attestations")).n);
